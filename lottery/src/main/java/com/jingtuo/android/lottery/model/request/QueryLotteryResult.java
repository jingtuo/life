...
public class QueryLotteryResult implements Callable<LotteryResultWrapper> {
    ...
    public QueryLotteryResult(String code, String time) {
        this.code = code;
        this.time = time;
    }
    ...
    public LotteryResultWrapper call() throws Exception {
        if (TextUtils.isEmpty(time)) {
            ...
            //优先保存到数据
            LotteryRepo.getInstance().getLotteryDao().insertLotteryResults(result);
            LotteryResult lotteryResult = result.get(0);
            result = LotteryRepo.getInstance().getLotteryDao().queryLotteryResults(code, lotteryResult.getTime());
        ...
        List<LotteryResult> result = LotteryRepo.getInstance().getLotteryDao().queryLotteryResults(code, time);
        if (result.size() < Constants.SIZE) {
            //本地数据不足,从服务器查询
